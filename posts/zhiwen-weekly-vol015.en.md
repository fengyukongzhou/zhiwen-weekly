---
title: "Vol.015 A Personal Experience"
date: 2026-01-11
tags:
  - Reading Fiction
  - Translation
...
<p align="right">— Rick Rubin, <i>The Creative Act</i></p>
...
<p align="right">— Mike Goldsmith, <i>Sound: A Very Short Introduction</i></p>
...
<p align="right">— <i>Sombrero Fallout</i></p>
...
<p align="right">— <i>The Abortion: An Historical Romance 1966</i></p>
...
<p align="right">— <i>The Hawkline Monster</i></p>

When you take your pill, it's like a **mine disaster**.
<p align="right">— <i>The Pill Versus the Springhill Mine Disaster</i></p>
...
<p align="right">— <i>Trout Fishing in America</i></p>
...
<p align="right">— <i>Dreaming of Babylon</i></p>
...
<p align="right">— <i>Revenge of the Lawn</i></p>
...
His own funeral flashed through his mind.
It was a beautiful funeral.
He saw this beautiful Japanese girl at his funeral, wearing a veil that perfectly complemented her dark eyes.
She walked just ahead of his coffin as it was carried toward the grave. She kept perfect pace with the coffin and the pallbearers on either side. His funeral was like a river flowing into eternity.
It couldn't stop.
They walked right past his grave.
They did not pause at that gaping hole in the earth.
Led by her, they simply kept walking, on and on, into forever.
<p align="right">— <i>Sombrero Fallout</i></p>
...
skirt, not particularly pretty,
is walking down the street.

A businessman stops, turns,
and stares at her ass.
Her ass looks like a
moldy refrigerator.

America
now has a population of 200,000,000.
<p align="right">— <i>The Pill Versus the Springhill Mine Disaster</i></p>
...
I've heard of [PaddleOCR](https://aistudio.baidu.com/paddleocr) for a long time, though it used to primarily require local deployment. Seeing that they've now launched an online version, I decided to give it a spin.

The workflow is largely identical to [MinerUOCR](https://mineru.net/OpenSourceTools/Extractor) and can be summarized as follows:

1. Use Acrobat or similar tools to split the PDF chapter by chapter.
2. Batch-drag the files into PaddleOCR for recognition (I used `PaddleOCR-VL`).
3. Download the recognized Markdown files into a single folder.
4. Use Gemini 3.0 Flash to proofread the text, setting the Temperature to 0 and the Thinking level to Medium.
5. Invoke Pandoc to merge the Markdown files and convert them into EPUB or other formats.
...
1. Use Acrobat or similar tools to split the PDF chapter by chapter.
2. Use Gemini 3.0 Pro, keeping the system prompt unchanged but lowering the Thinking level to Low.
3. **Open a new chat window for each chapter**; save the recognized results as Markdown files (or compile them directly into one file).
4. Invoke Pandoc to convert them into an EPUB.
...
**Stage 1: Preparation**
1. Select the Gemini 3.0 Pro / 3.0 Flash model in Google AI Studio.
2. Enter the translation prompt into the "System Instructions."

**Stage 2: Translation**
1. Open the EPUB file using the Calibre editor.
2. Open each `XHTML` file sequentially, copy all the text, and send it to the LLM for translation.
3. Save the translated result for each chapter as an independent Markdown file.

**Stage 3: Proofreading**
1. Use an AI CLI (or just use the Flash model in AI Studio directly) to check the Markdown files one by one.
2. Identify and correct any instances of mixed-language text.

**Stage 4: Repackaging**
1. Rename the original EPUB file to `.zip` and extract its contents.
2. Extract the images folder.
3. Place the images folder in the same directory as the translated Markdown files.
4. Use an AI CLI to invoke Pandoc: merge all Markdown files and convert them into a new EPUB file.

I built a version using Google AI Studio's Build mode ([Gemini EPUB Translator](https://ai.studio/apps/drive/1fet85vtv0GbwJ7T7fazNVCzTw4u5L6Fp)), which can call Gemini 3.0 Flash and execute the entire workflow with a single click:
...
If your functional requirements are more complex, you might be better off just using [AiNiee](https://github.com/NEKOparapa/AiNiee). Making my own app was primarily to cater to my specific needs, such as:
...
Subsequently, I forked this project and tweaked it into a version that allows for custom API calls: [AI EPUB Translator](https://ai.studio/apps/drive/1jtBovzliEfh0KWvlb37ZiakHIvVcVTJd). Now, it can be downloaded and run locally via npm.
